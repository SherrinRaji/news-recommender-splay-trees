...
class SplayTree:
    ...
    def __search_tree_helper(self, node, key):
        if node == None:
            return None
        if key == node.data or key in node.data:
            return node
        if key < node.data:
    ...
    def __pre_order_helper(self, node,splay_list=""):
        if node != None:
            # sys.stdout.write("{} ".format(node.data))
            splay_list="{}-{}".format(splay_list,node.data)
            splay_list=(self.__pre_order_helper(node.left,splay_list))
            splay_list=(self.__pre_order_helper(node.right,splay_list))
        return splay_list
